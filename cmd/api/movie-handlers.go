...
// MoviePayload структура выступает в качество DTO для post или put запросов
...
func getPoster(movie models.Movie) models.Movie {
	...
	req, err := http.NewRequest(http.MethodGet, theUrl+key+"&query="+url.QueryEscape(movie.Title), nil)
	if err != nil {
		log.Println(err)
		return movie
	}
